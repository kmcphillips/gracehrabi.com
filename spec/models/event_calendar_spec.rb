...
describe EventCalendar do
  ...
  describe "#table" do
    ...
    it "should layout a calendar" do
      expect(table.size).to eq(5)
      expect(table.first.first).to be_a(EventCalendar::EmptyCell)
      expect(table.first.last).to be_a(EventCalendar::Cell)
      expect(table.last.first).to be_a(EventCalendar::Cell)
      expect(table.last.last).to be_a(EventCalendar::EmptyCell)
    end
